...
As a first-generation dual major working towards a BSCS and BSDA, I aim to work as a software engineer and a data scientist/engineer. To outline my career goals in computer science, I would like to share projections from careers at www.computerscience.org. From “the Bureau of Labor Statistics (BLS) projects jobs for computer and information technology occupations will grow significantly — an 11% increase — from 2019 to 2029. This is significantly faster than the projected growth rate for all occupations nationwide. Some computer science jobs, such as computer and information research scientists, are projected to grow even more quickly.” (Red Ventures, 2022)

From this projection, I strongly believe that I am on course to achieve the knowledge and skills in the industry that are in high demand. I understand that both degrees and the concentrations afforded are unique. But I feel that each provides a necessary skill in the computer science industry. Being new to coding, SNHU has provided me with my first steps to developing code. I plan to pursue an internship or a position that can continue to teach me industry standards so that I may implement each skill as I learn. 
I know that I enjoy what software engineering affords with creating, editing, and updating software programs, databases, and applications. I also know that I enjoy analyzing information that is collected to share and interpret with heads of the organization. In blending the two, I have found that data engineers oversee and work with both programming and data frameworks. I aim to achieve the status of a data scientist/data engineer and believe this title utilizes the skills and knowledge of both BSCS and BSDA degrees. I would like to pursue this position now that I am learning code to develop algorithms that satisfy analytical reasoning for reports. At some point in my career, I would like to teach students the knowledge and skills based on my experience in the industry.
...
Reflecting on my experiences with completing course work and developing the ePortfolio. I believe each has helped me showcase strengths that have shaped my professional goals, preparing me for the computer science and data analytic fields. Artifact three: databases has showcased strengths in collaborating in a team environment, communicating to stakeholders, and software engineering and databases. Artifact one: software design and engineering has showcased strengths in data structures and algorithms. Artifact two: databases has showcased strengths in security. The areas that help address each strength are provided below in the narratives found within each artifact, each artifact may have overlapping strengths.

In Summarizing how each artifact fits the ePortfolio as a whole, I believe that the knowledge, skill, and ability to further enhance each artifact demonstrates my growth as a student in both fields. The launchpad simulates components of emerging systems to provide a working thermostat for an HVAC system. The encryption file simulates a secure transfer of data from one location to another and is stored to a new encrypted file. The rma orders generated are used to create a new database that extracts data which in turn is cleaned, sorted, structured, formatted, linked to shared attributes, and verified. All artifacts show unique skills in accessing information, resources, equipment, and or coding best practices to develop a working knowledge of skills and abilities learned within my experiences here at Southern New Hampshire University.
...
Although I would have preferred that the cool and heat functions had their own dedicated LED output of Red, Green, or Yellow. By maintaining access to the drivers of UART and I2C, the project is able to read the temperature output and record the temperature, setpoint, heat, cool, and seconds parameters. As a result, I have a Thermostat Application that adheres to coding standards, best practices, and illustrates the programming skills that have matured through my education at Southern New Hampshire University as a Computer Science, Software Engineering student.
...
Reflecting on the process of enhancing and modifying this artifact, I took many steps to prepare changes to the database structure, the fields, and their attributes. During this process I made multiple mistakes with correctly naming each table, field, and attribute. I made sure to take my time as I needed to key in each line manually(line item by line item). Using CAPS on commands and proper names from the Pseudocode became difficult. I found myself having to DESCRIBE and SHOW TABLES often to verify my entries, as there were times when SELECT * would not supply the required outcome. This was due to inaccurate entries on my part when creating the database and its fields. The same can be said when performing queries to pull data, requiring the use of proper syntax at each step. While creating the README.md file, I ensured best practices were implemented in the artifact and its documentation. I had fun creating the database and assigning the links to connect the data imported within the tables. The report indicates my findings, identifying where certain products have high volumes of returns. As well as certain states that show signs of higher percentages when it comes to purchases and returns overall. While I was not able to determine if the inclusion of climate was a benefactor to higher returns. The data indicates the highest frequencies come from colder climates.
...
The skills for this artifact demonstrate an ability to employ strategies for building collaborative environments that enable diverse audiences to support organizational decision making in the field of computer science. In the README.md file, instructions are available, taking the reader step by step on how to create a database, its tables, and fields. Along with the attributes that help maintain the data that is inserted. The README file contains commenting to guide the reader in understanding each step of the process. In addition to commenting and providing steps, visual illustrations are also provided to demonstrate key aspects of the artifact design. Ultimately, developing code that is readable and understandable to non-technical audiences. In communicating the steps of the artifact, once the data is collected and reports are used to analyze the data. The data points are used to create reports that define percentages and frequencies of purchases and returns. The README file is used to share the findings and analysis from the data collected on the RMA Database .


##### For example:
![Picture7](https://user-images.githubusercontent.com/79305154/152256249-fe31c4fa-a7d7-48fb-9128-3dccbed909d5.png)

The skills for this artifact demonstrate an ability to design, develop, and deliver professional-quality oral, written, and visual communications that are coherent, technically sound, and appropriately adapted to specific audiences and contexts. The artifact requires access to the csv files for customers, orders, and RMA. By communicating with the server and creating a functional database, collaborative efforts are achieved to share the data, its relationships, and an analysis based on findings in frequencies and percentages. By developing a new database and importing the necessary data values, the reports generated communicate visually how the historical events compare by state and by product regarding purchases and returns. In developing a report, available on the README page of the project, readers are able to identify the items in question regarding high volume purchases and returns. Different states, along with various products show unique patterns to purchase history and the issues that may have come from the equipment selected. The pseudocode developed illustrates an ability to communicate the desirable outcomes of the database structure and its functionality.

The skills for this artifact demonstrate an ability to use well-founded and innovative techniques, skills, and tools in computing practices for the purpose of implementing computer solutions that deliver value and accomplish industry-specific goals. Settings have been used to configure the database environment within Codio. The steps provided within the README.md file help to develop a database that is functional and is free of errors. I have added comments in code for the README.md file that adhere to best practices. I have made an effort to not over comment on items of this project, as a result, redundancies have been removed. I have ensured that there are no errors in logic, by developing code and determining the proper settings and resources used. The code developed is more robust and efficient, delivering value and accomplishing industry-specific goals. Testing the output of the query using limits helps to verify content before creating a large csv file from the datapoints selected. These documents that use specified variables further enhance this artifact. In checking the location of the file output and variables, performing this action can be helpful to determine if the query functions as expected.
...
Although I would have preferred to have more data available to link to the database, I was grateful to have found the climate data information from the www.energy.gov website. By creating a functional database and inserting the data to query, sort, and export results. I now have an RMA Database system that adheres to coding standards, best practices, and illustrates the programming skills that have matured through my education at Southern New Hampshire University as a Computer Science, Software Engineering student.
